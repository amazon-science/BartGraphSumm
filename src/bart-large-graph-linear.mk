...
BART_LARGE=$(HOME)/fairseq/models/bart.large.long.graph.linear/model.pt
OUTPUT_DIR=output
CHECKPOINT=$(OUTPUT_DIR)/checkpoint_best.pt

ARCH=bart_large_long_graph_linear
...
$(TASK)-bin: $(TASK)
	for SPLIT in train $(DEV_NAME); do \
	  for EXT in source target; do \
	    python3.6 -m examples.roberta.multiprocessing_bpe_encoder --encoder-json $(HOME)/fairseq/encoder-updated.json --vocab-bpe $(HOME)/fairseq/vocab.bpe --inputs "$(TASK)/$$SPLIT.$$EXT" --outputs "$(TASK)/$$SPLIT.bpe.$$EXT" --workers 60 --keep-empty; \
	  done; \
	done
...
#	mkdir -p $(OUTPUT_DIR)
#	date > $(OUTPUT_DIR)/time-start-freeze.txt
...
#	    --max-epoch 3 \
#        --freeze-rest-pattern 'embed_rank_positions' 'pos_proj' \
...
#        --extra-input $(TASK) \
#	  2>$(OUTPUT_DIR)/stderr.txt | tee $(OUTPUT_DIR)/stdout-freeze.txt
#	date > $(OUTPUT_DIR)/time-end-freeze.txt
